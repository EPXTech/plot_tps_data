...
def generate_lookup_table(avg_tps1, avg_tps2, filename='lookup_table.c'):
    with open(filename, 'w') as file:
        file.write('/* Lookup Table Generated from Average TPS Readings */\n')
        file.write('float avg_tps1[] = {' + ', '.join(f'{x:.2f}' for x in avg_tps1) + '};\n')
        file.write('float avg_tps2[] = {' + ', '.join(f'{x:.2f}' for x in avg_tps2) + '};\n')
...
def plot_data(files):
    ...
    generate_lookup_table(avg_tps1_percent, avg_tps2_percent)
